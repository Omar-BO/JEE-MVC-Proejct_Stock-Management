		...
		<div class="card">
			<div class="card-body" >
				<c:if test="${product != null}">
					<form action="product?page=update" method="post">
				</c:if>
				<c:if test="${product == null}">
					<form action="product?page=insert" method="post">
				</c:if>
				...
				<c:if test="${product != null}">
					<input type="hidden" name="id" value="<c:out value='${product.getIdPorduct()}' />" />
				</c:if>

				<fieldset class="form-group">
					<label>Label</label> <input type="text"
						value="<c:out value='${product.getLabel()}' />" class="form-control"
						name="label" required="required">
				</fieldset>
				
				<fieldset class="form-group">
					<label>Description</label> <input type="text"
						value="<c:out value='${product.getDescription()}' />" class="form-control"
						name="description" required="required" style="word-break: break-word;">
				</fieldset>
				...
				<fieldset class="form-group">
					<label>Price</label> <input type="number"
						...
						name="price" required="required" step="0.001" min="1">
				</fieldset>
				
				<fieldset class="form-group">
					<label>VAT</label> <input type="number"
						value="<c:out value='${product.getVAT()}' />" class="form-control"
						name="vat" required="required" step="0.01" min="0.1" max="0.99">
				</fieldset>
				...
				<fieldset class="form-group">
					...
					<select name="category" class="form-control" required>
						...
						<c:forEach items="${listCategory}" var="cat" >
							<option value="${cat['id']}"><c:out value="${cat['label']}"/></option>
						</c:forEach>
					</select>
				</fieldset>
				...
				<fieldset class="form-group">
					...
					<select name="provider" class="form-control" required>
						<option value="" disabled selected hidden>Please choose a Provider</option>
						<c:forEach items="${listProvider}" var="cat" >
